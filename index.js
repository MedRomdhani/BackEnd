const httpserver = require('http');
const fs = require('fs');
const port = 3001;
const htmlfile = fs.readFileSync('index.html');


let users =[
  {id: 90,username: 'Matthew Barrett', email: 'paf@co.va', phone: '5.75.214.104'},
  {id: 46,username: 'John Kim', email: 'vilo@pij.mw', phone: '241.63.58.191'},
  {id: 99,username: 'Bruce Frank', email: 'ispezkur@gaojsus.tl', phone: '3.60.66.240'},
  {id: 57,username: 'Marguerite Casey', email: 'lev@sehola.ax', phone: '125.30.184.41'},
  {id: 96,username: 'Alberta Reynolds', email: 'farreuw@duguk.py', phone: '253.72.102.59'},
  {id: 28,username: 'Augusta Garrett', email: 'gudkuldol@not.im', phone: '232.194.195.157'},
  {id: 100,username: 'Mary Wallace', email: 'tikkihsi@ku.bw', phone: '151.36.171.107'},
  {id: 54,username: 'Todd Potter', email: 'vullehub@re.vu', phone: '159.120.218.62'},
]


const server = httpserver.createServer((req, res) => {
  if(req.url == '/' && req.method == 'GET'){
    res.write("<a href='about'>go to About</a>");
  } else if(req.url == '/about' && req.method == 'GET'){
    res.setHeader('Content-Type', 'text/html');
    res.write("<img src='data:image/png;base64,'/>");
  } else if(req.url == '/users' && req.method == 'GET'){
    res.write(JSON.stringify(users));
  } else if(req.url == '/html' && req.method == 'GET'){
    res.write(htmlfile);
  } else if(req.url == '/adduser' && req.method == 'POST'){
    res.on('data', function(chunk){
      users.push(JSON.parse(chunk));
      res.write(chunk);
    })
    res.write(JSON.stringify(users));
  } else {
    res.write("Invalid request!!!!");
  }
  res.end("**************end of request******************");
});
server.listen(port, () => {
  console.log('server running on port 3001--------------------');
});